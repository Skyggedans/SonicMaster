...
void main() {
  ...
  // The on-pedal module on/off report (01020409) must flip the module's enabled
  // state immediately — the report used to be dropped, so an on-pedal ON never
  // relit the indicator.
  testWidgets('a module-enable report flips the module on/off state', (
    tester,
  ) async {
    final data = await tester.runAsync(DataAssets.load);
    ...
    final container = ProviderContainer(
      overrides: [
        deviceServiceProvider.overrideWith((_) => _StateDumpSpy(u01StateDump)),
        dataAssetsProvider.overrideWith((ref) => data!),
      ],
    );
    ...
    final name = data!.modules.nameOf(2)!;

    // Seed: module 2 currently OFF.
    container
        .read(currentPresetStateProvider.notifier)
        .state = DecodedPresetState(
      isCloneMode: false,
      presetVolume: 50,
      presetBpm: 120,
    ...
    // Device reports module 2 turned back ON (state byte 01).
    handleHardwareSync(
      ref,
      InboundMessage.classify(
        '8080F0000020001000000A0102040900020000000000000001000000000000F7',
      ),
    );

    expect(
      container.read(currentPresetStateProvider)!.moduleStates[name],
      isTrue,
      reason: 'the on-pedal module-ON report must relight the indicator',
    );
  });
}
