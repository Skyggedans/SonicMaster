...
final deviceServiceProvider = Provider<DeviceService>((ref) {
  ...
  final transport = createTransport(kind);
...
/// Available MIDI ports, one entry per direction (dev tooling).
final midiPortsProvider = FutureProvider<List<({String name, bool isInput})>>((
  ref,
) async {
  final devices = await enumerateMidiDevices();

  return [
    for (final d in devices) ...[
      if (d.hasInput) (name: d.name, isInput: true),
      if (d.hasOutput) (name: d.name, isInput: false),
    ],
  ];
});
