...
/// Prompts for a `.nam` to load into User-Profile [slot] (0-based). Returns its
/// path + display name, or null if the user cancels.
Future<({String path, String name})?> pickNamForClone(int slot) async {
...
/// Reads a picked `.nam` and checks it's a supported model format. Throws
/// [UnsupportedNamFormat] when the converter can't clone the architecture.
Future<String> readAndValidateNam(String path) async {
  final namJson = await File(path).readAsString();
