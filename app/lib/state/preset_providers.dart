...
import '../protocol/global_setting_writer.dart';
...
/// Re-applies the fields the reg-0x10 dump doesn't carry — Power Confirm, Batt
/// Only and the EXP/FS targets live in their own registers — so a re-read/poll
/// doesn't reset them after the user sets them (writes are bound; their read
/// isn't yet).
GlobalSettings _withUnreadable(GlobalSettings decoded, GlobalSettings? prev) =>
    prev == null
    ? decoded
    : decoded.copyWith(
        powerConfirm: prev.powerConfirm,
        battOnly: prev.battOnly,
        expFsTarget: prev.expFsTarget,
        expFsTarget2: prev.expFsTarget2,
      );
...
Future<void> refreshGlobalSettings(WidgetRef ref) async {
  ...
  try {
    ...
    final merged = _withUnreadable(g, ref.read(globalSettingsProvider));

    ref.read(globalSettingsProvider.notifier).state = merged;
    ref.read(globalVolumeProvider.notifier).state = merged.globalVolume;
  } catch (_) {
    // best-effort
  } finally {
    ref.read(presetLoadingProvider.notifier).state = false;
...
/// Address `(a0,a1)` of each device-global setting written via a synthesized
/// dumpRecord frame (bound from the 2026-07-24 labeled capture). Levels + master
/// volume keep their own captured-frame path ([setGlobalLevel]/[setGlobalVolume]).
const _globalSettingAddress = <String, (int, int)>{
  'mode': (0x04, 0x04),
  'reamp': (0x06, 0x04),
  'expFsType': (0x01, 0x05),
  'backlight': (0x03, 0x02),
  'eco': (0x04, 0x02),
  'powerConfirm': (0x05, 0x02),
  'battOnly': (0x06, 0x02),
};

/// Writes a device-global setting by synthesizing its frame ([name] is a key of
/// [_globalSettingAddress]), optimistically updating [globalSettingsProvider].
/// Not a preset edit. No-op mid-load. Mirrors [setGlobalLevel].
Future<void> setGlobalSetting(WidgetRef ref, String name, int value) async {
  if (ref.read(presetLoadingProvider)) return; // don't edit mid-load

  final addr = _globalSettingAddress[name];

  if (addr == null) return;

  ref.read(_globalEditAtProvider.notifier).state = DateTime.now();

  final frame = GlobalSettingWriter.dumpRecord(
    a0: addr.$1,
    a1: addr.$2,
    value: value,
  );

  // Capture notifiers + current value BEFORE the await: this runs from the
  // settings dialog, whose WidgetRef is disposed if it closes mid-send.
  final settings = ref.read(globalSettingsProvider.notifier);
  final status = ref.read(presetLoadStatusProvider.notifier);
  final current = ref.read(globalSettingsProvider);
  ...
  if (current != null) {
    settings.state = switch (name) {
      'mode' => current.copyWith(mode: value),
      'reamp' => current.copyWith(reamp: value),
      'expFsType' => current.copyWith(expFsType: value),
      'backlight' => current.copyWith(backlight: value),
      'eco' => current.copyWith(eco: value),
      'powerConfirm' => current.copyWith(powerConfirm: value),
      'battOnly' => current.copyWith(battOnly: value),
      _ => current,
    };
  }
}

/// Writes an EXP/FS Target; the current [GlobalSettings.expFsType] picks the
/// register/sub: EXP → reg 0x17 sub 01; SingleFS → reg 0x16 sub 00; DualFS →
/// reg 0x16 sub 02 (Func 1) or 04 (Func 2, [isFunc2]). Optimistic; no-op
/// mid-load. Not in the settings dump, so it's a write-only (optimistic) value.
Future<void> setExpFsTarget(
  WidgetRef ref,
  int value, {
  bool isFunc2 = false,
}) async {
  ...
  final status = ref.read(presetLoadStatusProvider.notifier);
  final current = ref.read(globalSettingsProvider);
  final type = current?.expFsType ?? 0;

  final (reg, sub) = switch (type) {
    0 => (0x17, 0x01), // EXP
    1 => (0x16, 0x00), // SingleFS
    _ => isFunc2 ? (0x16, 0x04) : (0x16, 0x02), // DualFS Func 1 / Func 2
  };

  ref.read(_globalEditAtProvider.notifier).state = DateTime.now();

  final frame = GlobalSettingWriter.registerWrite(
    reg: reg,
    sub: sub,
    value: value,
  );
  ...
  if (current != null) {
    settings.state = isFunc2
        ? current.copyWith(expFsTarget2: value)
        : current.copyWith(expFsTarget: value);
  }
}

/// Sends the Factory Reset command (write op `0x11`, selector `0x3F`, no payload,
/// verified byte-exact against the captured frame). The pedal ACKs, then reboots
/// and drops the link — the existing connection-drop handling takes over.
/// Fire-and-forget; the caller gates it behind a confirmation.
Future<void> factoryReset(WidgetRef ref) async {
  final frame = GlobalSettingWriter.action(selector: 0x3F);
  final status = ref.read(presetLoadStatusProvider.notifier);

  try {
    await ref.read(deviceServiceProvider).sendFrame(frame);
    status.state = 'factory reset — pedal rebooting';
  } catch (e) {
    status.state = 'reset failed: $e';
  }
}
